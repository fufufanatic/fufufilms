...
def get_films():
    ...
    # Customizable tmdb films (2019 and newer) query that populates a results list
    count = 1
    while tmdb_movies.top_rated(page=count)['results']:
        films = tmdb_movies.top_rated(page=count)['results']
        for film in films:
            if int(film['release_date'][0:4]) == 2008:
                print(film, '\n')
    ...
    for film in films:
        ...
        for film_rating in film_ratings: 
            ...
            if film_rating['Source'] == 'Metacritic':
                # For example, an Metacritic rating of 70/100 is returned as 70
